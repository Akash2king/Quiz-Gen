...
export async function generateQuestionAction(formData: FormData) {
  try {
    ...
    const result = await generateQuestion({ topic: validatedData.topic, count: validatedData.count });
    return { success: true, data: result };
  } catch (error) {
    console.error(error);
    if (error instanceof z.ZodError) {
      const fieldErrors = error.flatten().fieldErrors;
      const errorMessage = fieldErrors.topic?.[0] || fieldErrors.count?.[0] || 'Invalid input provided.';
